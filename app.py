...
# 1. Set Konfigurasi Halaman Utama
...
def load_pytorch_model():
    ...
    # KUSTOMISASI CLASSIFIER: Menyesuaikan layer linear agar cocok dengan file .pth kamu 
    # Untuk mengatasi error "size mismatch" ([576 -> 256 -> 5 kelas])
    model.classifier = nn.Sequential(
        nn.Linear(576, 256),       # Lapisan pertama: input 576, output 256
        nn.Hardswish(),
        nn.Dropout(p=0.2, inplace=True),
        nn.Linear(256, 5)          # Lapisan terakhir: output 5 kelas
    )
...
# Memuat model ke dalam aplikasi Streamlit
try:
    model = load_pytorch_model()
except Exception as e:
    st.error(f"Gagal memuat model. Pastikan file 'mobilenetv3_small_5kelas.pth' ada di folder yang sama. Error: {e}")

# 3. Daftar Label (Urutan disesuaikan persis dengan hasil evaluasi test set kamu)
labels = ['alpara', 'amoxcillin', 'lecozinc', 'simvastatin(QL)', 'simvastatin(selvim)']

# 4. Pipeline Transformasi Gambar (Normalisasi Diaktifkan)
preprocess = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.ToTensor(),
    # Mengaktifkan normalisasi standar ImageNet agar distribusi nilai piksel gambar
    # kamera sama dengan gambar dataset saat kamu training dulu.
...
if img_file_buffer is not None:
    ...
    with st.spinner('Sedang mengidentifikasi obat...'):
        ...
        with torch.no_grad():
            ...
            # Baris di bawah ini untuk debugging di terminal/log server kamu
            print("Probabilitas tiap kelas:", probabilities.tolist())
